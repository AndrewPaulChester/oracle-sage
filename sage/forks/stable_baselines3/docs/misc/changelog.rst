...
Breaking Changes:
^^^^^^^^^^^^^^^^^
- Removed ``sage.forks.stable_baselines3.stable_baselines3.common.cmd_util`` (already deprecated), please use ``env_util`` instead
...
- ``make_atari_env``, ``make_vec_env`` and ``set_random_seed`` must be imported with (and not directly from ``sage.forks.stable_baselines3.stable_baselines3.common``):

.. code-block:: python

  from sage.forks.stable_baselines3.stable_baselines3.common.cmd_util import make_atari_env, make_vec_env
